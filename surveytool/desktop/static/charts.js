...
// Cross-tab: one question x one demographic, rendered like an xbreak chart.
// Suppressed cells get a null bar value (never a misleading height) plus a
// visible label; greyed cells keep their real figure but are muted. n and
// the reason are always shown, never hidden.
function crossTabTraceAndLayout(result) {
  const labels = result.cells.map(c => c.subgroup_label);
  const values = result.cells.map(c => c.status === "suppressed" ? null : (c.t2b ?? c.mean));
  const colors = result.cells.map(c =>
    c.status === "suppressed" ? CHART_COLORS.suppressed :
    c.status === "grey" ? CHART_COLORS.grey : CHART_COLORS.ok
  );
  const patterns = result.cells.map(c => c.status === "suppressed" ? "/" : "");
  const text = result.cells.map(c =>
  ...
  return {
    data: [{
      ...
      hovertemplate: "%{x}: %{y:.1f}<br>n=%{customdata[0]} (%{customdata[1]})<extra></extra>",
    }],
    layout: withChartTemplate({
      title: `${result.question_text} by ${result.demographic}`,
      margin: { l: 60, r: 20, t: 40, b: 60 },
    }),
  };
}

async function loadCrossTabPickers(sessionId) {
  const crosstabResult = document.getElementById("crosstab-result");
  let response;
  try {
    response = await fetch(`/api/session/${sessionId}/demographics`);
  } catch {
    renderInlineError(crosstabResult, {
      code: "INTERNAL",
      message: "Could not reach the local server.",
      next_action: "Try again. If it keeps happening, restart the app.",
    });
  ...
  const body = await parseJsonResponse(response);
  if (!response.ok) {
    renderInlineError(crosstabResult, errorFromBody(body));
    return;
  }

  const questionSelect = document.getElementById("ct-question");
  const demographicSelect = document.getElementById("ct-demographic");
  ...
  body.demographics.forEach(d => {
    const opt = document.createElement("option");
    opt.value = d.key;
    opt.textContent = d.label;
    demographicSelect.appendChild(opt);
  });
}

async function loadCrossTab(sessionId, qid, demographic, container) {
  container.innerHTML = "Loading cross-tab…";
  let response;
  try {
    response = await fetch(
      `/api/session/${sessionId}/cross-tab?qid=${encodeURIComponent(qid)}&demographic=${encodeURIComponent(demographic)}`
    );
  } catch {
    renderInlineError(container, {
      code: "INTERNAL",
  ...
  const { data, layout } = crossTabTraceAndLayout(body);
  Plotly.newPlot(div, data, layout, { displaylogo: false, responsive: true });
}
